...
bias <- function(lambda, X, x0, beta) {
  ...
  ?solve
  value <- (lambda - t(x0)%*%(t(X)%*%X-lambda*diag(p))^-1%*%t(X)%*%X%*%beta)^2
  return(value)
}
...
variance <- function(lambda, X, x0, sigma) {
  ...
  print(inv)
  #print(t(x0)%*%inv%*%t(X))
  #print(2 * t((t(X)%*%X + lambda * diag(p))^-1%*%t(X))%*%x0)
  value <- t(x0)%*%inv%*%t(X) %*% (sigma^2 * diag(nrow(X))) %*% t((t(X)%*%X + lambda * diag(p))^-1%*%t(X))%*%x0
  print(value)
  return(value)
}
lambdas <- seq(0, 2, length.out = 500)
...
ggplot(Salaries, aes(x = sex, y = salary, color = rank)) +
  geom_jitter() +
...
SVAR TIL B):
A possibility is that the relationship between sex and salary is confounded with other variables in the full model. This might happen when another variable is associated with both the predictor (sex) and the outcome (salary), causing the apparent relationship between the predictor and outcome to 'lie'. To try and demonstrate this, we first created a a scatter-plot which shows the salary of men and women, with different colors representing the different levels of rank. We also created a box-plot of the salary for men and women, but without consideration of the rank. The box-plot clearly shows that there's a difference between the salary for men and women. Though when we compare the box-plot to the scatter-plot, we see..........  
...
table(glm.pred)
# 441 sights classified as clear sightings.
```
...
Using the summary(lm.bigfoot), we can see that the odds of an observation being classified Class A increases by 28% if the report contains the word "saw".
...
# 626 sights classified as clear sightings.
```
...
There are no predefined statiscial models for choosing $k$.I would create a plot between different $k$ values and their errors. Based on this plot i would choose the $k$ that leads to the lowest error.
...
plot(roc_glm, col="blue", lwd=3, main="ROC curve GLM")
auc(roc_glm)
...
knnMod
roc_knn <- roc(response = test$class, predictor = knnProb)
plot(roc_knn, col="green", lwd=3, main="ROC curve KNN")
auc(roc_knn)
```
...
QDA and logistic regression performs pretty equal and are bot good choices.WRITE MORE.
